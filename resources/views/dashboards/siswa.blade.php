...
<div class="space-y-8 animate-fade-in">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="group relative bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            <div class="absolute -right-2 -bottom-4 w-24 h-24 bg-red-500/5 group-hover:bg-red-500/10 rounded-full transition-colors duration-300 pointer-events-none -z-10"></div>
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl bg-red-500/10 text-red-600 dark:text-red-405 flex items-center justify-center font-semibold shrink-0">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 leading-none mt-1">{{ $jumlahTugasBelumSelesai }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="group relative bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 leading-none mt-1">{{ rtrim(rtrim(number_format((float)$nilaiRataRata, 2, '.', ''), '0'), '.') }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="group relative bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 leading-none mt-1">{{ $kehadiran }}%</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                ...
                <span class="text-xxs font-bold text-red-600 bg-red-500/10 px-2.5 py-1 rounded-full uppercase">Penting</span>
            </div>
            <div class="p-6 overflow-y-auto max-h-[350px] custom-scrollbar">
                <div class="space-y-6">
                    @forelse($tugasMendatang as $t)
                        <div class="flex items-center justify-between gap-4 p-4 rounded-xl border {{ $t['is_urgent'] ? 'border-red-200/30 dark:border-red-500/10 bg-red-50/20 dark:bg-red-500/5' : 'border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50' }}">
                            <div class="flex items-start gap-3">
                                <span class="w-2.5 h-2.5 rounded-full {{ $t['is_urgent'] ? 'bg-red-500' : 'bg-amber-500' }} mt-1 shrink-0"></span>
                                <div>
                                    <h4 class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ $t['judul'] }}</h4>
                                    <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ $t['mapel'] }} • Batas: {{ $t['diff'] }}</p>
                                </div>
                            </div>
                            <a href="{{ route('tugas.show', [$t['kelas_id'], $t['id']]) }}" class="px-3.5 py-1.5 {{ $t['is_urgent'] ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700' }} text-white rounded-lg text-xs font-semibold shadow transition-colors">
                                Kerjakan
                            </a>
                        </div>
                    @empty
