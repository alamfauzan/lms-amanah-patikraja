<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.siswa.index') }}" class="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
            </a>
            <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100 leading-tight">Edit Data Siswa</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden animate-fade-in">
            <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                <h3 class="font-bold text-slate-800 dark:text-slate-100">Ubah Informasi Siswa</h3>
                <p class="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Ubah nama, email, atau reset password siswa.</p>
            </div>

            <form method="POST" action="{{ route('admin.siswa.update', $siswa->id) }}" class="px-8 py-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $siswa->email) }}" required
                           placeholder="Contoh: budi@lms.com"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="password" id="password" name="password"
                                   ...
                                   class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-855 border border-slate-200 dark:border-slate-700/60 rounded-xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 @error('password') border-red-400 @enderror">
                            @error('password')
                        ...
                        <div>
                            ...
                            <input type="password" id="password_confirmation" name="password_confirmation"
                                   ...
                                   class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-855 border border-slate-200 dark:border-slate-700/60 rounded-xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <a href="{{ route('admin.siswa.index') }}" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition duration-200">
                        Batal
                    </a>
                </div>
            </form>
