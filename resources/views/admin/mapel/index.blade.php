<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100 leading-tight">Mata Pelajaran</h2>
    ...
    <div class="space-y-6">
        ...
        @if($mapels->isEmpty())
            <div class="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl text-center shadow-sm">
                ...
                <h3 class="text-base font-bold text-slate-800 dark:text-slate-200 mb-1">Belum ada mata pelajaran</h3>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200/80 dark:border-slate-700/80 text-slate-500 dark:text-slate-400">
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Nama Mata Pelajaran</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Kode Mata Pelajaran</th>
                                <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider">Tanggal Dibuat</th>
                                ...
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                                    <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs shrink-0 shadow-inner">
                                                {{ strtoupper(substr($mapel->nama_mapel, 0, 2)) }}
                                            </div>
                                            <span>{{ $mapel->nama_mapel }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="inline-block px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-750 dark:text-slate-300 font-mono text-xs rounded-lg border border-slate-200/50 dark:border-slate-700/50">
                                            {{ $mapel->kode_mapel }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-slate-400 dark:text-slate-500">{{ $mapel->created_at->format('d M Y') }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.mapel.destroy', $mapel->id) }}" onsubmit="return confirm('Yakin ingin menghapus mata pelajaran ini?')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
