<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100 leading-tight">Tambah Tahun Ajaran</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden animate-fade-in">
            <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                <h3 class="font-bold text-slate-800 dark:text-slate-100">Informasi Tahun Ajaran</h3>
                <p class="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Isi periode tahun ajaran baru.</p>
            </div>

            <form method="POST" action="{{ route('admin.tahun-ajaran.store') }}" class="px-8 py-6 space-y-6">
                @csrf
