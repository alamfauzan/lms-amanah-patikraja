    ...
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <div class="flex items-center gap-3">
                <a href="{{ route('kelas.pertemuan.show', [$materi->kelas->id, $materi->pertemuan->id]) }}" class="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                </a>
                <div>
                    <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100 truncate max-w-md sm:max-w-xl">{{ $materi->judul }}</h2>
            ...
            @if(auth()->user()->hasAnyRole(['admin','guru']))
                <div class="flex items-center gap-2">
                    <a href="{{ route('materi.edit', $materi->id) }}"
                       class="px-3.5 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 text-slate-600 dark:text-slate-350 hover:text-indigo-600 dark:hover:text-indigo-400 text-xs font-semibold rounded-xl transition duration-200">
                        Edit
                    </a>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-6 border-b border-slate-100 dark:border-slate-800/50 flex flex-wrap items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-900/50">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-indigo-500/10 text-indigo-650 dark:text-indigo-400 flex items-center justify-center font-bold text-sm">
                ...
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider
                        {{ $materi->tipe === 'video' ? 'bg-red-500/10 text-red-550' : ($materi->tipe === 'file' ? 'bg-amber-500/10 text-amber-555' : 'bg-blue-500/10 text-indigo-500') }}">
            ...
            <div class="p-6">
                @if($materi->tipe === 'teks')
                    <!-- Text Content -->
                    <div class="prose dark:prose-invert max-w-none text-slate-700 dark:text-slate-350 leading-relaxed space-y-4">
                ...
                @elseif($materi->tipe === 'file')
                    ...
                    <div class="max-w-lg mx-auto py-8">
                        <div class="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl p-6 text-center space-y-4 shadow-inner">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-400 mt-1">Silakan unduh dokumen di bawah ini untuk mempelajari materi pelajaran.</p>
                            </div>
                            <div class="pt-2">
                ...
                @elseif($materi->tipe === 'video')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 rounded-xl flex items-center justify-between">
                            <div class="min-w-0">
                                <p class="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate">{{ basename($materi->file_path) }}</p>
                                <p class="text-[10px] text-slate-400">Gunakan tombol pemutar video untuk mulai menonton.</p>
                            </div>
                            <a href="{{ asset('storage/' . $materi->file_path) }}" download
                               class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-350 text-xs font-semibold rounded-lg transition duration-200 shrink-0">
