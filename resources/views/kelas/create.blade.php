<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100 leading-tight">Tambah Kelas Baru</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                <h3 class="font-bold text-slate-800 dark:text-slate-100">Informasi Kelas</h3>
                <p class="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Isi detail kelas yang akan dibuat.</p>
            </div>

            <form method="POST" action="{{ route('kelas.store') }}" class="px-8 py-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                              ...
                              class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 resize-none">{{ old('deskripsi') }}</textarea>
                </div>
