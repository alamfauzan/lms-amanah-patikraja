<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-bold text-lg text-slate-800 dark:text-slate-100">Edit Pertemuan</h2>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800">
                <h3 class="font-bold text-slate-800 dark:text-slate-100">Edit Pertemuan</h3>
                <p class="text-sm text-slate-400 mt-0.5">Kelas: {{ $kelas->nama_kelas }}</p>
            </div>
            <form method="POST" action="{{ route('kelas.pertemuan.update', [$kelas->id, $pertemuan->id]) }}" class="px-8 py-6 space-y-5">
                @csrf @method('PUT')
                <div>
                    ...
                    <select id="mata_pelajaran_id" name="mata_pelajaran_id"
                        ...
                        @foreach($mapels as $mapel)
                            <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $pertemuan->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $pertemuan->judul) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="urutan" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Urutan <span class="text-red-500">*</span></label>
                        <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $pertemuan->urutan) }}" min="1"
                               class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                    </div>
                    <div>
                        <label for="tanggal" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Tanggal</label>
                        <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', optional($pertemuan->tanggal)->format('Y-m-d')) }}"
                               class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                    </div>
                </div>
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                              class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition resize-none">{{ old('deskripsi', $pertemuan->deskripsi) }}</textarea>
                </div>
                <div class="flex items-center gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <a href="{{ route('kelas.pertemuan.index', [$kelas->id, 'mapel_id' => $pertemuan->mata_pelajaran_id]) }}" class="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-600 dark:text-slate-300 text-sm font-semibold rounded-xl transition">Batal</a>
                </div>
            </form>
        </div>
    </div>
