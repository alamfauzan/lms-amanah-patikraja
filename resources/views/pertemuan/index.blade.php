                                ...
                                <div class="flex items-center gap-2">
                                    @if(auth()->user()->hasAnyRole(['admin','guru']))
                                        <a href="{{ route('kelas.pertemuan.edit', [$kelas->id, $item->id]) }}"
                                           class="p-1.5 text-slate-400 hover:text-indigo-650 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-lg transition-colors">
                                        ...
                                        <form method="POST" action="{{ route('kelas.pertemuan.destroy', [$kelas->id, $item->id]) }}" onsubmit="return confirm('Hapus pertemuan ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="p-1.5 text-slate-400 hover:text-red-650 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors">
                            ...
                            <div class="space-y-3">
                                ...
                                @else
                                    ...
                                    @foreach($item->materi as $m)
                                        @php
                                            if ($m->tipe === 'video') {
                                                $itemIconBg = 'bg-red-500/10 text-red-655 dark:bg-red-500/20 dark:text-red-400';
                                                ...
                                                $itemIconBg = 'bg-indigo-500/10 text-indigo-650 dark:bg-indigo-500/20 dark:text-indigo-400';
                                        ...
                                        @endphp
                                        <a href="{{ route('materi.show', $m->id) }}" class="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/80 rounded-xl hover:shadow-sm transition duration-200">
                                    ...
                                    @foreach($item->tugas as $t)
                                        <a href="{{ route('tugas.show', [$kelas->id, $t->id]) }}" class="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/80 rounded-xl hover:shadow-sm transition duration-200">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <h4 class="text-sm font-bold text-orange-605 dark:text-orange-400 mt-0.5 hover:underline truncate">{{ $t->judul }}</h4>
                                            </div>
                                        </a>
                                    @endforeach
                                    ...
                                    @foreach($item->kuis as $k)
                                        <a href="{{ route('kuis.show', [$kelas->id, $k->id]) }}" class="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800/80 rounded-xl hover:shadow-sm transition duration-200">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <h4 class="text-sm font-bold text-emerald-655 dark:text-emerald-400 mt-0.5 hover:underline truncate">{{ $k->judul }}</h4>
                                            </div>
                                        </a>
                                    @endforeach
                                @endif
                            ...
                            @if(auth()->user()->hasAnyRole(['admin','guru']))
                                <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2">
                                    <a href="{{ route('kelas.pertemuan.materi.create', [$kelas->id, $item->id]) }}"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-650 dark:text-indigo-400 hover:bg-indigo-100 text-xs font-semibold rounded-lg transition-colors">
                                    ...
                                    <a href="{{ route('kelas.kuis.create', $kelas->id) }}"
                                       class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-655 dark:text-emerald-400 hover:bg-emerald-100 text-xs font-semibold rounded-lg transition-colors">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                                        Buat Kuis
                                    </a>
                                </div>
